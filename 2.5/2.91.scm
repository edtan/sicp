  ...
  (if (same-variable? (variable p1) (variable p2))
      (let ((div-result (div-terms (term-list p1)
            ...
            (make-poly (variable p1)
                       (add-terms (car div-result) (car div-result)))))
  ...
  (if (empty-termlist? L1)
      ...
      (let ((t1 (first-term L1))
        ...
        (if (> (order t2) (order t1))
            ...
            (let ((new-c (div (coeff t1) (coeff t2)))
                  (new-o (- (order t1) (order t2))))
              (let ((rest-of-result (div L1
                                         (sub L1
                                              (mul-term-by-all-terms (make-term new-o new-c) L2)))))   ;  assumes that we have installed an arithmetic  package for terms
                (list (append (make-dense-term new-o new-c) (car rest-of-result))
                      (cdr rest-of-result))
                ))))))